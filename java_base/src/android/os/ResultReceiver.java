...
public class ResultReceiver implements Parcelable {
    ...
    interface IResultReceiver extends IInterface{
        public void send(int resultCode, Bundle resultData) throws RemoteException;
    }
    
    class MyResultReceiver extends Binder implements IResultReceiver{

        @Override
        protected boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
            send(code, data.readBundle());
            return true;
        }
    ...
    ResultReceiver(Parcel in) {
        ...
        IInterface iface = binder.queryLocalInterface("IResultReceiver");
        if (iface == null){
            mReceiver = new IResultReceiver() {
                @Override
                public void send(int resultCode, Bundle resultData) throws RemoteException{
                    Parcel data = Parcel.obtain();
                    Parcel reply = Parcel.obtain();
                    resultData.writeToParcel(data, 0);
                    binder.transact(resultCode, data, reply, 0);
                }
            ...
            mReceiver = (IResultReceiver)iface;
        }
    }
