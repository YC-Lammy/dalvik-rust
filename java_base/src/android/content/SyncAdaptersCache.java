...
public class SyncAdaptersCache extends RegisteredServicesCache<SyncAdapterType> {
    ...
    public SyncAdapterType parseServiceAttributes(Resources res,
        ...
        TypedArray sa = res.obtainAttributes(attrs,
                android.R.styleable.SyncAdapter);
        try {
            final String authority =
                    sa.getString(android.R.styleable.SyncAdapter_contentAuthority);
            final String accountType =
                    sa.getString(android.R.styleable.SyncAdapter_accountType);
            if (TextUtils.isEmpty(authority) || TextUtils.isEmpty(accountType)) {
                return null;
            }
            final boolean userVisible =
                    sa.getBoolean(android.R.styleable.SyncAdapter_userVisible, true);
            final boolean supportsUploading =
                    sa.getBoolean(android.R.styleable.SyncAdapter_supportsUploading,
                            true);
            final boolean isAlwaysSyncable =
                    sa.getBoolean(android.R.styleable.SyncAdapter_isAlwaysSyncable,
                            false);
            final boolean allowParallelSyncs =
                    sa.getBoolean(android.R.styleable.SyncAdapter_allowParallelSyncs,
                            false);
            final String settingsActivity =
                    sa.getString(android.R.styleable
