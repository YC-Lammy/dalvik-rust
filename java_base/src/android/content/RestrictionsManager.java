 ...
 * manages the useandroid.R. Apps can also send permission requests to a local or remote
 * device administrator to override default app-specific restrictions or any other
 * operation that needs explicit authorization from the administratoandroid.R.
 * <p>
 ...
 * Restrictions Provideandroid.R. The Restrictions Provider can respond back to requests by calling
 * {@link #notifyPermissionResponse(String, PersistableBundle)}, when
 * a response is received from the administrator of the device or useandroid.R.
 ...
 * not be shown to the administratoandroid.R. It can be used to pass along data that cannot be modified,
...
public class RestrictionsManager {
     ...
     * available. If there's no cached response, it will issue a new one to the administratoandroid.R.
     * <p>
     * Type: boolean
     */
     ...
     * be available in {@link #RESPONSE_KEY_MESSAGE}, to be displayed to the useandroid.R.
     * <p>
     * Type: int
     * <p>
    ...
    /**
     * Error code indicating that there was an internal erroandroid.R.
     * <p>
     * Stored in {@link #RESPONSE_KEY_ERROR_CODE} field in the response bundle.
     */
    ...
    static native Bundle nGetApplicationRestrictions(String packageName);
    static native boolean nHasRestrictionsProvider();
    static native void nRequestPermission(String packageName, String requestType, String requestId, PersistableBundle requestData);
    static native void nNotifyPermissionResponse(String packageName, PersistableBundle response);
    static native Intent nCreateLocalApprovalIntent();
    ...
    public Bundle getApplicationRestrictions() {
        return nGetApplicationRestrictions(mContext.getPackageName());
    }

    /**
     * Called by an application to check if there is an active Restrictions Provideandroid.R. If
     * there isn't, {@link #requestPermission(String, String, PersistableBundle)} is not available.
     *
     * @return whether there is an active Restrictions Provideandroid.R.
     */
    public boolean hasRestrictionsProvider() {
        return nHasRestrictionsProvider();
    }
    ...
    public void requestPermission(String requestType, String requestId, PersistableBundle request) {
        ...
        nRequestPermission(mContext.getPackageName(), requestType, requestId,
                        request);
    }

    public Intent createLocalApprovalIntent() {
        return nCreateLocalApprovalIntent();
    }
    ...
    public void notifyPermissionResponse(String packageName, PersistableBundle response) {
        ...
        nNotifyPermissionResponse(packageName, response);
    }
        ...
        if (xml.getName().equals(TAG_RESTRICTION)) {
            ...
            if (attrSet != null) {
                TypedArray a = appContext.obtainStyledAttributes(attrSet,
                        android.R.styleable.RestrictionEntry);
    ...
    private RestrictionEntry loadRestriction(Context appContext, TypedArray a, XmlResourceParser xml)
            throws IOException, XmlPullParserException {
        String key = a.getString(android.R.styleable.RestrictionEntry_key);
        int restrictionType = a.getInt(
                android.R.styleable.RestrictionEntry_restrictionType, -1);
        String title = a.getString(android.R.styleable.RestrictionEntry_title);
        String description = a.getString(android.R.styleable.RestrictionEntry_description);
        int entries = a.getResourceId(android.R.styleable.RestrictionEntry_entries, 0);
        int entryValues = a.getResourceId(android.R.styleable.RestrictionEntry_entryValues, 0);
        ...
        switch (restrictionType) {
            ...
            case RestrictionEntry.TYPE_CHOICE:
                restriction.setSelectedString(
                        a.getString(android.R.styleable.RestrictionEntry_defaultValue));
                break;
            case RestrictionEntry.TYPE_INTEGER:
                restriction.setIntValue(
                        a.getInt(android.R.styleable.RestrictionEntry_defaultValue, 0));
                break;
            case RestrictionEntry.TYPE_MULTI_SELECT:
                int resId = a.getResourceId(android.R.styleable.RestrictionEntry_defaultValue, 0);
                if (resId != 0) {
            ...
            case RestrictionEntry.TYPE_BOOLEAN:
                restriction.setSelectedState(
                        a.getBoolean(android.R.styleable.RestrictionEntry_defaultValue, false));
                break;
