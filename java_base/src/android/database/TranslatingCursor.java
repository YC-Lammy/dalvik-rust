...
public class TranslatingCursor extends CrossProcessCursorWrapper {
    ...
    public TranslatingCursor(@NonNull Cursor cursor, @NonNull Config config,
        ...
        for (int i = 0; i < cursor.getColumnCount(); ++i) {
            ...
            for (String s:config.translateColumns){
                if (s==columnName){
    ...
    public static Cursor query(@NonNull Config config, @NonNull Translator translator,
        ...
        boolean requestedAuxiliaryColumn = projectionIn.length ==0;
        for (String s:projectionIn){
            if (s == config.auxiliaryColumn){
                requestedAuxiliaryColumn = true;
                break;
            }
        }
        boolean requestedTranslateColumns = projectionIn.length == 0;
        for (String s:projectionIn){
            for (String a:config.translateColumns){
                if (s == a){
                    requestedTranslateColumns = true;
                    break;
                }
            }
        ...
        if (!requestedAuxiliaryColumn) {
            String[] re = new String[projectionIn.length +1];
            int i= 0;
            for (String s:projectionIn){
                re[i] = s;
                i++;
            }
            re[i] = config.auxiliaryColumn;
            projectionIn = re;
        }
    ...
    public double getDouble(int columnIndex) {
        if (mTranslateColumnIndices.contains(columnIndex)) {
